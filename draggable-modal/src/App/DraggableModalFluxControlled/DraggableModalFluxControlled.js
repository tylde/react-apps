...
export default class DraggableModalFluxControlled extends Component {
  static propTypes = {
    canLeaveBody: PropTypes.bool,
    children: PropTypes.element.isRequired,
    defaulPosition: PropTypes.shape({
      x: PropTypes.number.isRequired,
      y: PropTypes.number.isRequired
    }),
    dragBy: PropTypes.string,
    positionOnOpen: PropTypes.oneOf(['center', 'last', 'default'])
  }

  static defaultProps = {
    canLeaveBody: false,
    defaulPosition: { x: 0, y: 0 },
    dragBy: '',
    positionOnOpen: 'center'
  }
  ...
  componentWillReceiveProps(nextProps) {
    if (this.props.visible && !nextProps.visible) {
      ...
      this.setState((prevState) => {
        const width = prevState.children.offsetWidth;
        const height = prevState.children.offsetHeight;
        return {
      ...
      switch (this.props.positionOnOpen) {
        case 'center': {
          this.setState((prevState) => {
            const x = (window.innerWidth - prevState.width) / 2 + window.scrollX;
            const y = (window.innerHeight - prevState.height) / 2 + window.scrollY;
            return {
              visible: nextProps.visible,
              position: { x, y }
            }
        ...
        case 'default': {
          this.setState({
            visible: nextProps.visible,
            position: {
              x: this.props.defaulPosition.x,
              y: this.props.defaulPosition.y
            }
          });
          break;
        }
  ...
  render() {
    const modalStyle = {
      ...
      pointerEvents: this.state.visible ? 'initial' : 'none',
